 ...
 * dependent only on the interface; the implementation here still uses the SamplerViewModel but can
 * be replaced by other implementations in tests or different environments.
 */
class ViewModelAudioPreviewGenerator(private val sampler: SamplerViewModel = SamplerViewModel()) :
