    ...
    class ImportPipelineTest {
        ...
        fun setup() {
            ...
            db = Room.inMemoryDatabaseBuilder(context, MediaDb::class.java)
        ...
        fun import_createsNeptuneZip_andStoresOnlyProjectUri() = runBlocking {
            ...
            val entries = ZipFile(project).use { zf ->
                zf.entries().toList().map { it.name }.toSet()
            }
            assertTrue("config.json missing from zip", "config.json" in entries)
            assertTrue("audio file missing from zip",
                entries.any { it.endsWith(".wav") || it.endsWith(".mp3") })
        ...
        fun import_deletesTempAudioCopy_ifUseCaseConfiguredToDoSo() = runBlocking {
            // The fake importer writes to cacheDir. After import, ImportMediaUseCase deletes the temp audio.
