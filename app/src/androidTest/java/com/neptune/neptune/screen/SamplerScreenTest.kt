...
class FakeSamplerViewModel : SamplerViewModel() {
    ...
    override fun updateAttack(value: Float) { isAttackUpdated = true; super.updateAttack(value) }
    override fun selectTab(tab: SamplerTab) { isSelectTabCalled = tab; super.selectTab(tab) }
    override fun togglePlayPause() { isTogglePlayPauseCalled = true; super.togglePlayPause() }
    override fun saveSampler() { isSaveSamplerCalled = true }
    override fun increasePitch() { isIncreasePitchCalled = true; super.increasePitch() }
    override fun decreasePitch() { isDecreasePitchCalled = true; super.decreasePitch() }
    override fun updateTempo(newTempo: Int) { lastTempoUpdated = newTempo; super.updateTempo(newTempo) }
    override fun updatePlaybackPosition(position: Float) { lastPlaybackPosition = position; super.updatePlaybackPosition(position) }

}

class SamplerViewModelFactory(private val viewModel: FakeSamplerViewModel) : ViewModelProvider.Factory {
...
class SamplerScreenTest {
    ...
    fun samplerTabs_clickEQ_switchesTabContent() {
        ...
        fakeViewModel.mutableUiState.value = fakeViewModel.uiState.value.copy(currentTab = SamplerTab.EQ)
        composeTestRule.waitForIdle()
    ...
    fun saveButton_callsSaveSampler() {
        composeTestRule.onNodeWithContentDescription(saveButtonDesc).assertHasClickAction().performClick()
    ...
    fun playhead_drag_callsUpdatePlaybackPosition() {
        ...
        assertTrue("updatePlaybackPosition should have been called after dragging the playhead.", fakeViewModel.lastPlaybackPosition != null)
    }
}