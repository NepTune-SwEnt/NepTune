...
class ProfileScreenTest {
  ...
  fun viewModeDisplaysNameUsernameBioAndStats() {
    ...
    composeTestRule
        .onNodeWithTag(ProfileScreenTestTags.NAME)
        .assertIsDisplayed()
        .assert(hasText("Jane Roe"))

    composeTestRule
        .onNodeWithTag(ProfileScreenTestTags.USERNAME)
        .assertIsDisplayed()
        .assert(hasText("@janeroe"))

    // Bio is wrapped with quotes in view mode: “ <bio> ”
    composeTestRule
        .onNodeWithTag(ProfileScreenTestTags.BIO)
        .assertIsDisplayed()
        .assert(hasText("“ Hello world ”"))

    composeTestRule.onNodeWithTag(ProfileScreenTestTags.FOLLOWERS_BLOCK).assert(hasText("42"))

    composeTestRule.onNodeWithTag(ProfileScreenTestTags.FOLLOWING_BLOCK).assert(hasText("7"))
  }
  ...
  fun editFieldsUpdateAndSaveReceivesTrimmedValues() {
    ...
    composeTestRule
        .onNodeWithTag(ProfileScreenTestTags.FIELD_USERNAME)
    ...
    // Here we assert that the callback received *some* values (integration with VM covered in unit
    // tests).
  ...
  fun usernameValidationErrorDisablesSave() {
    ...
    // We can still assert it's present; enable/disable state is a Material property not directly
    // exposed by test APIs,
  ...
  fun bioCharacterCounterIsShownWhenNoError() {
    ...
    // We can at least assert the field is visible; the counter text itself is part of
    // supportingText,
