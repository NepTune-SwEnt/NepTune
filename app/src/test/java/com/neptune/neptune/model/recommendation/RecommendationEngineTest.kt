...
class RecommendationEngineTest {
    ...
    private fun userWithTags(
        uid: String,
        vararg pairs: Pair<String, Double>
    ): RecoUserProfile =
        RecoUserProfile(
            uid = uid,
            tagsWeight = mapOf(*pairs)
        )
    ...
    fun tagSimilarityEmptyUserOrSampleReturnsZero() {
        assertEquals(
            0.0,
            RecommendationEngine.tagSimilarity(emptyMap(), listOf("a", "b")),
            1e-9
        )

        assertEquals(
            0.0,
            RecommendationEngine.tagSimilarity(mapOf("tag" to 1.0), emptyList()),
            1e-9
        )
    }
    ...
    fun scoreSamplePrefersTagMatchingSamples() {
        ...
        val user = userWithTags(
            uid = "user_tag_matching",
            "miku" to 3.0,
            "lofi" to 1.0
        )
    ...
    fun rankSamplesForUserRespectsLimit() {
        ...
        val result = RecommendationEngine.rankSamplesForUser(
            user = user,
    ...
    fun rankSamplesForUserFloatsTagRelevantSamplesToTop() {
        ...
        val ranked = RecommendationEngine.rankSamplesForUser(
            user = user,
            candidates = listOf(irrelevant, relevant),
            limit = 10
        )
