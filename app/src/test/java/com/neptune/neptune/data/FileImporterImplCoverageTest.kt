...
class FileImporterImplCoverageTest {
    ...
    private fun writeTinyWav(dst: File, seconds: Double = 0.2, sampleRate: Int = 8000) {
        ...
        dst.outputStream().use { out ->
            fun w32(v: Int) = out.write(byteArrayOf(
                (v and 0xFF).toByte(), ((v shr 8) and 0xFF).toByte(),
                ((v shr 16) and 0xFF).toByte(), ((v shr 24) and 0xFF).toByte()))
            fun w16(v: Int) = out.write(byteArrayOf(
                (v and 0xFF).toByte(), ((v shr 8) and 0xFF).toByte()))

            out.write("RIFF".toByteArray()); w32(chunkSize); out.write("WAVE".toByteArray())
            out.write("fmt ".toByteArray()); w32(16); w16(1) // PCM
            w16(numChannels); w32(sampleRate); w32(byteRate); w16(blockAlign); w16(bitsPerSample)
    ...
    class TestAudioProvider : android.content.ContentProvider() {
        ...
        override fun update(u: Uri, v: android.content.ContentValues?, s: String?, a: Array<out String>?) = 0
    }
