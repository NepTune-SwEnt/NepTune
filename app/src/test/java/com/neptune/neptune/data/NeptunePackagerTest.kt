...
class NeptunePackagerTest {
    ...
    fun projectFile_numbers_equal_existing_count_and_falls_back_if_taken() {
        ...
        // If someone already created "a-3.zip", count=3, prefer "a-3.zip" but if taken, fall to "a-4.zip"
    ...
    fun creates_zip_with_config_and_audio() {
        ...
        // create an input wav
        val input = File(ctx.cacheDir, "in.wav").apply {
            outputStream().use { FileOutputStream(this).write(ByteArray(48) { 0x55 }) }
        }

        val out = packager.createProjectZip(
            audioFile = input,
            durationMs = 1200L
        )
